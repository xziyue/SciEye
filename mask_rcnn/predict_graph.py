...
from mask_rcnn.graph_dataset import register_graph_dataset
...
def do_predict2(pred_func, input_file):
    ...
    # print('number of boxes:', len(results))
    # if cfg.MODE_MASK:
    #     final = draw_final_outputs_blackwhite(img, results)
    # else:
    #     final = draw_final_outputs(img, results)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict-output', help="where to save the output prediction", nargs=1)
    ...
    if args.visualize:
        ...
        if args.predict:
            assert args.predict_output
            predictor = OfflinePredictor(predcfg)
            result = []
            for image_file in args.predict:
                result.append(do_predict2(predictor, image_file))
            joblib.dump(result, args.predict_output[0])
        elif args.evaluate:
